...
int8 ItemInterface::CanEquipItemInSlot(int8 DstInvSlot, int8 slot, ItemPrototype* proto, bool ignore_combat /* = false */, bool skip_2h_check /* = false */)
{
	...
	if((slot < INVENTORY_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET) || (slot >= BANK_SLOT_BAG_START && slot < BANK_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET))
	{
		...
		{
			Standing current_standing = Player::GetReputationRankFromStanding(m_pOwner->GetStanding(proto->RequiredReputationFaction));
			if(current_standing < (Standing)proto->RequiredReputationRank)	   // Not enough rep rankage..
...
int8 ItemInterface::CanAffordItem(ItemPrototype * item,uint32 amount, Creature * pVendor)
{
	...
	{
		FactionDBC *factdbc = dbcFaction.LookupEntry(item->RequiredReputationFaction);
		if(!factdbc || factdbc->RepListId < 0)
			return (int8)NULL;
		
		if( m_pOwner->GetReputationRankFromStanding( m_pOwner->GetStanding( item->RequiredReputationFaction)) < (int32)item->RequiredReputationRank)
		{
