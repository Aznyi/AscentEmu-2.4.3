...
#define SAFE_FREE_PTR(x) if(deletedPointers.find((x)) == deletedPointers.end()) { deletedPointers.insert((x)); free((x)); }
    ...
    for(i = 0; i < maxid; ++i)
	{
		for(unordered_map<uint32, LocalizedQuest>::iterator itr = m_Quests[i].begin(); itr != m_Quests[i].end(); ++itr)
		{
		...
		for(unordered_map<uint32, LocalizedItem>::iterator itr = m_Items[i].begin(); itr != m_Items[i].end(); ++itr)
		{
			SAFE_FREE_PTR(itr->second.Name);
			SAFE_FREE_PTR(itr->second.Description);
		}

		for(unordered_map<uint32, LocalizedNpcText>::iterator itr = m_NpcTexts[i].begin(); itr != m_NpcTexts[i].end(); ++itr)
		{
			for(j = 0; j < 8; ++j)
			{
		...
		for(unordered_map<uint32, LocalizedCreatureName>::iterator itr = m_CreatureNames[i].begin(); itr != m_CreatureNames[i].end(); ++itr)
		{
			SAFE_FREE_PTR(itr->second.Name);
			SAFE_FREE_PTR(itr->second.SubName);
		}

		for(unordered_map<uint32, LocalizedGameObjectName>::iterator itr = m_GameObjectNames[i].begin(); itr != m_GameObjectNames[i].end(); ++itr)
		{
			SAFE_FREE_PTR(itr->second.Name);
		}

		for(unordered_map<uint32, LocalizedItemPage>::iterator itr = m_ItemPages[i].begin(); itr != m_ItemPages[i].end(); ++itr)
		{
...
void LocalizationMgr::Reload(bool first)
{
	...
	m_CreatureNames = new unordered_map<uint32, LocalizedCreatureName>[langid];
	m_GameObjectNames = new unordered_map<uint32, LocalizedGameObjectName>[langid];
	m_Quests = new unordered_map<uint32, LocalizedQuest>[langid];
	m_NpcTexts = new unordered_map<uint32, LocalizedNpcText>[langid];
	m_Items = new unordered_map<uint32, LocalizedItem>[langid];
	m_ItemPages = new unordered_map<uint32, LocalizedItemPage>[langid];
...
#define MAKE_LOOKUP_FUNCTION(t, hm, fn) t * LocalizationMgr::fn(uint32 id, uint32 language) { \
	...
	unordered_map<uint32, t>::iterator itr = hm[language].find(id); \
