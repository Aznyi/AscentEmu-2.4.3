...
public:
	...
	template<typename T>
		void CopyHashMap(unordered_map<uint32,T> * src, unordered_map<uint32,T> * dest)
	{
		for(typename unordered_map<uint32,T>::iterator itr = src->begin(); itr != src->end(); ++itr)
			dest->insert(make_pair(itr->first,itr->second));
	}

private:
	unordered_map<uint32, LocalizedQuest>				* m_Quests;
	unordered_map<uint32, LocalizedItem>				* m_Items;
	unordered_map<uint32, LocalizedNpcText>			* m_NpcTexts;
	unordered_map<uint32, LocalizedCreatureName>		* m_CreatureNames;
	unordered_map<uint32, LocalizedGameObjectName>		* m_GameObjectNames;
	unordered_map<uint32, LocalizedItemPage>			* m_ItemPages;
