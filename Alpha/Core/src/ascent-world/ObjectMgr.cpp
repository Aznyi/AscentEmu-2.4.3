...
ObjectMgr::~ObjectMgr()
{
	...
	for(unordered_map<uint32, TimedEmoteList*>::iterator i = m_timedemotes.begin(); i != m_timedemotes.end(); ++i)
	{
	...
	{
		for(unordered_map<uint32, Charter*>::iterator itr =  m_charters[i].begin(); itr != m_charters[i].end(); ++itr)
		{
			delete itr->second;
		}
	}
	...
	for(unordered_map<uint32,InstanceReputationModifier*>::iterator itr = this->m_reputation_instance.begin(); itr != this->m_reputation_instance.end(); ++itr)
	{
...
void ObjectMgr::LoadCreatureTimedEmotes()
{
	...
	do
	{
		...
		unordered_map<uint32,TimedEmoteList*>::const_iterator i;
...
Transporter * ObjectMgr::GetTransporter(uint32 guid)
{
	...
	unordered_map<uint32, Transporter*>::const_iterator itr = mTransports.find(guid);
...
Transporter * ObjectMgr::GetTransporterByEntry(uint32 entry)
{
	Transporter * rv = 0;
	_TransportLock.Acquire();
	unordered_map<uint32, Transporter*>::iterator itr = mTransports.begin();
...
Charter * ObjectMgr::GetCharterByItemGuid(uint64 guid)
{
	...
	for(int i = 0; i < NUM_CHARTER_TYPES; ++i)
	{
		unordered_map<uint32, Charter*>::iterator itr = m_charters[i].begin();
...
Charter * ObjectMgr::GetCharterByName(string &charter_name, CharterTypes Type)
{
	Charter * rv = 0;
	m_charterLock.AcquireReadLock();
	unordered_map<uint32, Charter*>::iterator itr = m_charters[Type].begin();
...
ArenaTeam * ObjectMgr::GetArenaTeamByGuid(uint32 guid, uint32 Type)
{
	...
	for(unordered_map<uint32,ArenaTeam*>::iterator itr = m_arenaTeamMap[Type].begin(); itr != m_arenaTeamMap[Type].end(); ++itr)
	{
...
{
	unordered_map<uint32, ArenaTeam*>::iterator itr;
	m_arenaTeamLock.Acquire();
...
{
	m_arenaTeamLock.Acquire();
	for(unordered_map<uint32, ArenaTeam*>::iterator itr = m_arenaTeams.begin(); itr != m_arenaTeams.end(); ++itr)
	{
...
void ObjectMgr::UpdateArenaTeamRankings()
{
	...
	for(uint32 i = 0; i < NUM_ARENA_TEAM_TYPES; ++i)
	{
		...
		for(unordered_map<uint32,ArenaTeam*>::iterator itr = m_arenaTeamMap[i].begin(); itr != m_arenaTeamMap[i].end(); ++itr)
