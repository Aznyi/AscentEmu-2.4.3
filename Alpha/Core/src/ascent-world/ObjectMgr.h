...
typedef unordered_map<string, PlayerInfo*> PlayerNameStringIndexMap;
...
public:
    ...
    typedef unordered_map<uint32, Transporter*>                TransportMap;
	typedef unordered_map<uint32, Trainer*>                    TrainerMap;
	typedef unordered_map<uint32, std::vector<TrainerSpell*> > TrainerSpellMap;
    typedef unordered_map<uint32, ReputationModifier*>         ReputationModMap;
    typedef unordered_map<uint32, Corpse*>                     CorpseMap;
	...
	void LoadArenaTeams();
	unordered_map<uint32, ArenaTeam*> m_arenaTeamMap[3];
	unordered_map<uint32, ArenaTeam*> m_arenaTeams;
...
protected:
	...
	unordered_map<uint32, Charter*> m_charters[NUM_CHARTER_TYPES];
	...
	unordered_map<uint32,TimedEmoteList*> m_timedemotes;//stored by spawnid
