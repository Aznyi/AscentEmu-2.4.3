...
void Spell::SummonGuardian(uint32 i) // Summon Guardian
{
	...
	for( int i = 0; i < damage; i++ )
	{
		...
		u_caster->m_SummonSlots[m_summonProperties->slot < 7 ? m_summonProperties->slot : 0] = (Creature*)u_caster->create_guardian(cr_entry, GetDuration(), m_fallowAngle, level);
	}
}
...
{
     // This could be used to solve BE quests issues?
}
