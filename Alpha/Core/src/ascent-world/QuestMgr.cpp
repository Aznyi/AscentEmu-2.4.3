...
uint32 QuestMgr::PlayerMeetsReqs(Player* plr, Quest* qst, bool skiplevelcheck)
{
	...
	if(qst->required_max_rep_faction)
...
{
	unordered_map<uint32, list<QuestAssociation *>* > &associationList = GetQuestAssociationList();
	...
	if (associationList.find( itemId ) == associationList.end() )
	{
		...
		associationList.insert(unordered_map<uint32, list<QuestAssociation *>* >::value_type(itemId, tempList));
	}
	else
	{
...
{
	unordered_map<uint32, QuestAssociationList* > &associationList = GetQuestAssociationList();
	unordered_map<uint32, QuestAssociationList* >::iterator itr = associationList.find( itemId );
