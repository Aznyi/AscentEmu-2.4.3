...
void WorldSession::HandleItemQuerySingleOpcode( WorldPacket & recv_data )
{
	...
	data << itemProto->RequiredSkill;          // id from SkillLine.dbc
	...
	data << uint32(itemProto->Duration); 	      // Duration(seconds) 0 = permanent
	SendPacket( &data );
}
...
void WorldSession::HandleInsertGemOpcode(WorldPacket &recvPacket)
{
	...
	for(uint32 i = 0;i<TargetItem->GetSocketsCount();i++)
	{
		...
		if(EI)
		{
			...
			if(gp && !(gp->SocketMask & TargetItem->GetProto()->Socket[i].Color))
				ColorMatch=false;
		}
		...
		if(gemguid)//add or replace gem
		{
			...
			if(!gp)
				continue;
			if(!(gp->SocketMask & TargetItem->GetProto()->Socket[i].Color))
				ColorMatch=false;
