...
public:
	...
	typename unordered_map<uint32, T*> _map;
	...
	{
		for(typename unordered_map<uint32, T*>::iterator itr = _map.begin(); itr != _map.end(); ++itr)
			delete itr->second;
	}
	...
	bool DeallocateEntry(uint32 Entry)
	{
		typename unordered_map<uint32, T*>::iterator itr = _map.find(Entry);
		if(itr == _map.end())
			return false;
	...
	void Clear()
	{
		typename unordered_map<uint32, T*>::iterator itr = _map.begin();
...
class SERVER_DECL HashMapStorageIterator : public StorageContainerIterator<T>
{
	...
	typename unordered_map<uint32, T*>::iterator itr;
public:
