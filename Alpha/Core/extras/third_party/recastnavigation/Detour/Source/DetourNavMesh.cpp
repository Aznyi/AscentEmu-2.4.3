...
dtStatus dtNavMesh::init(const dtNavMeshParams* params)
{
	...
	// Legacy TBC mmap datasets can legitimately trade salt bits for higher
	// per-tile polygon budgets on continent-scale maps. Ascent's runtime
	// loads static tiles from disk and does not depend on a large salt space
	// the way long-lived highly dynamic navmeshes do, so keep the historical
	// 32-bit layout viable as long as we still have a non-trivial salt range.
	if (m_saltBits < 6)
